...
genesis_no_token_name = (
    "6a04534c500001010747454e45534953036162634c004c004c00010001020800000000000003e8"
)
...
test_ui_string_chunks = [
    b"SLP\x00",
    b"\x01",
    b"SEND",
    b"\xae\x88_\x8e\x1b\xd2\xa0\x9c_\x8d\xcf\x19\xcb\x9a\x83}\x8cH\xb3p\x19\x0f1\x84\x1b\x8aH\x17\xeb\xdb\xb9\xd7",
...
def test_mint_op_return_invalid_tokenid_length():
    with pytest.raises(SlpSerializingError):
        result = s.buildMintOpReturn(
            token_id_hex="F1", baton_vout=None, token_mint_quantity=1000, token_type=1
        )
...
def test_send_type_1():
    result = s.buildSendOpReturn(
        token_id_hex=mint_token_id, output_qty_array=[5, 10, 7], token_type=1
    )
...
def test_send_type_129():
    result = s.buildSendOpReturn(
        token_id_hex=mint_token_id, output_qty_array=[50, 1, 71], token_type=129
    )
...
def test_send_0_outputs():
    with pytest.raises(SlpSerializingError):
        result = s.buildSendOpReturn(
            token_id_hex=mint_token_id, output_qty_array=[], token_type=129
        )
...
def test_send_invalid_tokenid_length():
    with pytest.raises(SlpSerializingError):
        result = s.buildSendOpReturn(
            token_id_hex="F1", output_qty_array=[5], token_type=129
        )
